...
| T-57 | E2E: full P0 CLI parity test | done | | T-19, T-20, T-22, T-30, T-50, T-51, T-55 | `tests/e2e/test_m4_p0_parity.py` — 10 journeys covering init / memory CRUD+quick / validate / review / status / context pack / conformance / adapter / inbox send-ack-resolve-reject / mcp serve+install / migrate / pool subscribe-list-unsubscribe / observe+observer status / distill+propose review / doctor / config / version / wisdom. Expanding coverage surfaced + fixed a real bug: `distill`/`propose` ignored the global `--dir` flag (called `find_project_root()` directly) and crashed with a `ProjectNotFoundError` traceback on no-project; now route through `cfg.resolve_project_root()`, which raises a clean ClickException ("run `engram init`") for every command. |
...
| ID | Task | Status | Owner | Depends on | Notes |
|----|------|--------|-------|------------|-------|
| T-160 | `engram memory quick "<body>"` one-line command (issue #2) | done | | T-19 | name/description auto-derived from body (first line ≤80 chars / first 150 chars); `--type` defaults to `project`; `--scope` defaults to `project`; collisions get `-1/-2/...` suffix; auto-derived assets must validate clean [reconciled 2026-06-14: engram memory quick shipped.]|
| T-161 | `engram init --adopt` default for existing `.memory/` (issue #1) | done | | T-17 | Detects SPEC-compliant `.memory/`; skips MEMORY.md write; scans `local/`/`workflows/`/`kb/` for valid frontmatter and registers to graph.db; invalid files become warnings, not errors [reconciled 2026-06-14: engram init --adopt shipped.]|
...
| ID | Task | Status | Owner | Depends on | Notes |
|----|------|--------|-------|------------|-------|
| T-180 | graph.db canonical URI (issue #4) | partial | | T-14, T-161 | `engram/core/uri.py`: ✓ CanonicalURI dataclass + build_canonical_uri + parse_canonical_uri + resolve_store_root_id (reuses inbox SPEC §10.6 three-tier resolution: explicit config > git remote SHA > path hash). 11 unit tests. ⏳ graph.db schema migration (add `canonical_uri` column + backfill + change PK) deferred to T-186 v0.2.1 migrate so v0.2 → v0.2.1 is one atomic step. URI tool ready for use by Evolve Engine + cross-machine wisdom now |
...
| ID | Task | Status | Owner | Depends on | Notes |
|----|------|--------|-------|------------|-------|
| T-200 | observer 协议 + `engram observe` CLI | done | | T-15 | `engram/observer/` 多文件:queue / protocol / cli。stdin 读 JSON 入 `~/.engram/observe-queue/<session>.jsonl`,fcntl.flock + os.O_APPEND 单 write 原子。p99 < 50 ms,队列满返回 ok=false 不阻塞 hook。永不 import 客户端 SDK。 [reconciled 2026-06-14: observer protocol + engram observe shipped.]|
| T-201 | observer daemon 骨架 | done | | T-200 | `engram/observer/daemon.py` 单例 lockfile (`~/.engram/observer.pid`)。fcntl.flock LOCK_EX\ [reconciled 2026-06-14: observer daemon shipped.]|LOCK_NB,死进程 steal + 接管孤儿队列。watcher 轮询(POSIX 通用,inotify/FSEvents 留待 web UI 复用 T-112)。SIGTERM 优雅关停。 |
| T-202 | Tier 0 mechanical compactor | done | | T-200, T-201 | `engram/observer/tier0.py` 永远跑无 LLM:从 queue jsonl 提取 tool name / file paths / errors / 时间戳 / token,写 `<session>.timeline.jsonl`。每条 ≤ 4 KB(超出 truncate),prompt/stderr 正文 不入 timeline 只入 raw(默认 30 天 TTL,opt-in 保留)。失败模式:不会失败。 [reconciled 2026-06-14: Tier 0 mechanical compactor shipped.]|
| T-203 | Session 资产类型 + SPEC §3.x | done | | T-12, T-202 | SPEC.md 加 §3.x Session 章节 + frontmatter schema(type:session / session_id / client / started_at / ended_at / task_hash / tool_calls / files_touched / outcome / prev_session / next_session / distilled_into / scope:project / enforcement:hint)。`engram/core/frontmatter.py` 加 SESSION 枚举。Session 写到 `.memory/sessions/<YYYY-MM-DD>/sess_<id>.md`。validate 通过 + git diff 可读。 [reconciled 2026-06-14: Session asset type shipped.]|
| T-204 | Tier 1 LLM compactor + 离线降级 | done | | T-202, T-203 | `engram/observer/{providers/{base,ollama,openai_compatible},config,tier1,runners}.py`。Provider 是 `Callable[[str], str]` TypeAlias(plain def/lambda 直接当 provider)。3 种 provider:mechanical(MECHANICAL_MARKER 哨兵)/ ollama(POST /api/generate stream=false)/ openai-compatible(POST /chat/completions,自动追加路径,API key 支持 `$ENV` 展开)。`config.toml [observer.compactor.tier1]` 缺失/解析错误/loader 异常都降级到 mechanical(永不 panic)。Tier 1 失败模式:provider 抛 ProviderError / 返回 marker / 返回空字符串 → Tier 0 mechanical 渲染兜底。`compact_to_session_asset` 写 Session 资产到 `<project>/.memory/sessions/<date>/sess_<id>.md`(有 project_root)或 `~/.engram/sessions/<date>/sess_<id>.md`。`make_tier1_runner` 注入到 ObserverDaemon。55 单测,整库 893。零 SDK 依赖,纯 stdlib urllib。 |
| T-205 | 5 adapter hook 脚本 + `engram observer install` | done | | T-200 | 5 个 hook 脚本 `adapters/<client>/hooks/engram_observe_post_tool_use.sh`(claude-code / codex / cursor / gemini-cli / opencode)+ `engram/observer/translators.py`(Claude Code + Codex hook payload → engram event 映射)+ `engram/observer/install.py`(注册表 + plan + apply,write/paste 两种 action)+ `engram observer install --target=<client>` CLI(claude-code 写 ~/.claude/settings.json PostToolUse,其余 paste)。`engram observe --from=<client>` 增加翻译路径。无 hook 客户端走 raw-api 直接传 engram-format JSON。`--list` / `--dry-run` / 幂等(重装不重复写)。31 单测。 |
| T-206 | Relevance Gate Stage 0 | done | | T-40, T-203 | DESIGN §5.1 加 Stage 0(`select_session_continuations` 纯函数 + 集成进 `run_relevance_gate`):输入 task_hash → 同 hash 最近 ≤ 3 个 session(按 ended_at 倒序,未结束的排后)→ 加入 `RelevanceResult.sessions` + `sessions_tokens`,budget 上限 25%(`stage0_budget_fraction` 可调)。`engram/observer/loader.py` 把磁盘 Session 资产 hydrate 成 `SessionContinuation`。Stage 0 与现有阶段独立,task_hash 缺省时完全无副作用。15 单测。 |
| T-207 | Cross-session task linkage | done | | T-203, T-173 | `engram/observer/linkage.py`:`find_predecessor`(同 hash 最近 ended_at 早于 self.started_at,排除 self_id)/ `set_next_session`(fcntl.flock LOCK_EX + write_atomic 原子重写)/ `link_session_to_predecessor`(同时写 prev + 回填 next,任一失败不抛)。Tier 1 `compact_to_session_asset` 在 `task_hash` 给定时自动调用,`with suppress(Exception)` 守护 —— linkage 失败永远不杀 Tier 1。E2E:跨 2 session 写完任务链完整(`prev_session`/`next_session` 双向)。15 单测。 |
| T-208 | Tier 2 semantic distiller | done | | T-204 | `engram/observer/tier2.py` 5 函数式入口(`load_session_for_distill` / `select_sessions_for_distill` / `build_distill_prompt` / `distill_sessions` / `run_tier2`)。Provider 沿用 T-204 抽象,JSON 数组解析(strip ```json fences,跳过缺字段的 entry,slug 去重,cap 6)。失败链:provider 异常 → empty / 无效 JSON → empty → mechanical fallback。Mechanical fallback 选 ≥2 session 触达过的 file 生成 `recurring-files` 候选,**永不幻觉**(只引用真实 file path)。distilled 文件 Memory 兼容 frontmatter(`type: agent` / `scope: project` / `enforcement: hint` + `source_sessions`),所以 `engram distill promote` 后直接成为合法 Memory 资产。distilled 文件不进 Relevance Gate(目录与 local/ 隔离)。21 单测。 |
| T-209 | distill review/promote/reject | done | | T-208 | `engram/commands/distill.py` `engram distill {review,promote,reject}`。promote 把 `.memory/distilled/<name>.proposed.md` 移到 `.memory/local/<name>.md`,可通过 `--scope` / `--enforcement` 在落盘时 override。背向链路:promote 后扫 sessions/(project + 用户全局)对每个 source session 追加 `distilled_into`(原子 write_atomic + frontmatter 字段保留)。reject 归档到 `~/.engram/archive/distilled/<YYYY-MM>/<name>.<ts>[-reason].md`(SPEC §1.2 6 月最低保留)。`--dry-run` / `--format=json`。LLM 也能 consent —— 跑命令就是显式动作。17 单测。 |
| T-210 | Tier 3 procedural recognizer | done | | T-208 | `engram/observer/tier3.py`(propose_procedures / build_procedure_prompt / run_tier3 + ProcedureProposal 数据类)+ `engram/commands/propose.py` `engram propose review/promote/reject`。LLM JSON 解析与 T-208 同形(strip fences / dedupe slug / cap 4)。Mechanical fallback 按 task_hash 分组,只在"≥ 3 个 session + ≥ 2 完成"时产候选(高 leverage 资产 = 严格门槛,recall 优先于 precision)。promote 把 `proposal.md` 升级为 Workflow scaffold(README.md + spine.toml placeholder + metrics.yaml + fixtures/),reject 整个目录归档到 archive/workflows/。23 单测。 |
| T-211 | Confidence-driven decay + C7/C8 wisdom curves | done | | T-46, T-203 | `engram/observer/decay.py` 纯函数 `compute_effective_ttl_days` + `decide_decay`(SPEC §7.1 公式:base 30d + exposure*3d cap 60d - contradicted*5d - abandoned 14d,floor 7d)。`engram/wisdom/curves.py` 加 C7(continuation hit rate = exposure_count > 0 的 session 比例/天)+ C8(distillation yield = distilled_into 非空的 session 比例/天)。两曲线扫 .memory/sessions/ + ~/.engram/sessions/ 双 root 去重。21 单测。**注**:Consistency Engine Phase 4 跨 session 矛盾扫描留作 T-48r 范围,此处只做 decay + curves。 |
| T-212 | E2E 跨 5 session 验收 | done | | T-200~T-211 | `tests/e2e/test_auto_continuation_e2e.py` 完整流水:5 client(claude-code / codex / cursor / raw-api 离线 / claude-code)→ 同一 task_hash → 5 段 enqueue + Tier 0 + Tier 1 → Stage 0 注入(s5 命中头部)→ Tier 2 蒸馏 → distill promote 反向链回写 distilled_into → wisdom C7/C8 显示 yield > 0。1 E2E 测试,Hermetic(HOME 隔离 + 测试用 stub provider,无网络)。 |
...
| ID | Task | Status | Owner | Depends on | Notes |
|----|------|--------|-------|------------|-------|
| T-70 | `engram/workflow/` module + CLI subcommands: add / run / revise / promote / rollback / list / test | done | | T-13, T-14 | Workflow directory structure per SPEC §6; `add` creates skeleton [reconciled 2026-06-14: engram/workflow/ module + CLI (add/list/read/validate/run/test/history/revise/rollback/deprecate) shipped (SPEC §5).]|
| T-71 | `engram/workflow/runner.py` — spine execution (python / bash / toml) with sandbox | done | | T-70 | `toml` spine is declarative steps; `python` + `bash` run in subprocess; captures stdout/stderr [reconciled 2026-06-14: spine runner for python3/bash/toml in engram/workflow/runner.py; exceptions/timeouts -> failure outcome.]|
| T-72 | `engram/workflow/fixtures.py` — fixture harness | done | | T-70, T-71 | Loads `fixtures/`; runs spine; diffs actual vs expected output [reconciled 2026-06-14: fixture harness + 4 assertion types + outcome.tsv in engram/workflow/fixtures.py.]|
| T-73 | `engram/workflow/rev.py` — rev / current symlink management, git-native | done | | T-70, T-71 | Each revision is a git commit; `current` symlink points to active rev [reconciled 2026-06-14: rev/current copy-on-write + scaffold/snapshot/rollback in engram/workflow/rev.py.]|
