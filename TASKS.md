...
| T-35 | E2E: v0.1 store migration with real 20-memory sample | todo | | T-34 | Sample fixture in `tests/fixtures/v0.1_store/`; assert zero data loss |
| T-36 | Update `engram init` to accept `--subscribe=<pool>` and `--org` / `--team` | todo | | T-17, T-30, T-33 | Writes `pools.toml` entries; creates scope symlinks |
| T-37 | `pools.toml` schema validation in `engram validate` | todo | | T-20, T-30 | Validates `subscribed_at` values; checks pool path exists |
| T-38 | Scope-aware relevance ranking in `engram memory search` | todo | | T-19, T-30, T-33 | Applies enforcement weighting (mandatory > default > hint) in BM25 results |
| T-39 | Tests for scope conflict resolution per SPEC §8.4 decision tree | todo | | T-30, T-33, T-38 | 15+ conflict scenarios; each asserts the winning asset and reason |
...
| ID | Task | Status | Owner | Depends on | Notes |
|----|------|--------|-------|------------|-------|
| T-40 | `engram/relevance/gate.py` — DESIGN §5.1 7-stage pipeline | todo | | T-14 | Stages: prefilter → scope filter → BM25 → vector → temporal boost → enforcement weight → budget pack |
...
| T-42 | `engram/relevance/bm25.py` + stop word list | todo | | T-40 | Okapi-BM25; 32-term stop word list per DESIGN §17 |
| T-43 | `engram/relevance/temporal.py` — "N weeks ago" parsing + date boost | todo | | T-40 | Up to 40% distance reduction for sessions near query date |
