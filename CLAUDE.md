# CLAUDE.md — LLM collaborator guide for engram
...
Any LLM (Claude, Codex, Gemini, Cursor, local models) that opens this
...
`~/.engram/`. It targets the gap between "scratch notes in CLAUDE.md"
...
Global language rule (`~/.claude/rules/language.md`): ban Chinese
...
- Never write CLAUDE.md / AGENTS.md sections that talk down to the
...
任何打开这个仓库来帮忙开发的 LLM(Claude、Codex、Gemini、Cursor、本地
...
存成 `.memory/` 和 `~/.engram/` 下的普通 markdown 文件。它瞄准"CLAUDE.md
...
全局语言规则(`~/.claude/rules/language.md`):禁 矩阵 / 赋能 / 抓手 /
...
- 永远不在 CLAUDE.md / AGENTS.md 里写俯视读者的话。这些文件是写给同侪的。
