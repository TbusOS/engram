...
**最后更新**：2026-04-25（状态滞后,见下方提示）
**规范版本**：https://github.com/TbusOS/engram/blob/main/TASKS.md

> ⚠️ **这份中文任务板滞后于英文 [TASKS.md](TASKS.md)(英文为权威）。** 本文件停在 T-194,缺少 T-200–T-212(自动续接)整段,且多数任务状态停留在 2026-04-25,早于 M4.6/M5/M6/M7 的交付。看真实进度请以英文 TASKS.md 为准(已于 2026-06-27 对齐到 HEAD `81a9b6f`)。完整中文同步是一项独立待办。
...
| T-171 | 所有写入路径接入 usage bus | partial | | T-170 | `context.py` ✓ 每个 included asset emit `loaded_only` 事件 + `co_assets` 真实填;`consistency/resolve.py` ✓ DISMISS 动作 emit `false_positive_dismissed`;`validate.py` ⏳ 推迟到 T-184(mandatory directive)以确保只在有意义的任务边界发事件而不是每次 validate。3 wire-in 集成测试,整库 711 |
| T-172 | trust_weight 表权威定义入 SPEC §11.4 | done | | T-170 | 草案 `docs/superpowers/specs/2026-04-25-spec-amend-v0.2.1-usage-evidence.md` 覆盖 schema + 8 类 trust_weight 默认 + co_assets 平摊 + DESIGN §5.1 confidence_multiplier + migration。最终 SPEC.md / DESIGN.md 改动进 T-185 v0.2.1 PR |
| T-173 | task_hash 自动从 git context 派生 | done | | T-170 | `engram/usage/task_hash.py::derive_task_hash` 4 级优先:explicit > ENGRAM_TASK_HASH env > git HEAD+branch SHA-256 前缀 > 15 分钟时间窗 bucket(`tw-<hex>`)。CLI / MCP / context.py 都不要求用户填 task_hash。7 单测 |

#### 第 3-4 周 — v0.2.1 SPEC-AMEND PR 包(6 条 issue 一次合)

| ID | 任务 | 状态 | Owner | 依赖 | Notes |
|----|------|------|-------|------|-------|
| T-180 | graph.db canonical URI(issue #4) | partial | | T-14, T-161 | `engram/core/uri.py`:✓ CanonicalURI dataclass + build_canonical_uri + parse_canonical_uri + resolve_store_root_id(复用 inbox SPEC §10.6 三级解析:explicit config > git remote SHA > path hash)。11 单测。⏳ graph.db schema 迁移(加 `canonical_uri` 列 + backfill + 改主键)留给 T-186 v0.2.1 migrate 一起做(v0.2 → v0.2.1 单步原子升级)。URI 工具现在可以给 Evolve Engine + 跨机 wisdom 直接用 |
...
| T-188 | `engram wisdom report` ASCII 6 曲线 | done | | T-170 | `engram/wisdom/` 多文件(DESIGN §4.2):types(Curve / Sample / WisdomReport) + curves.py(6 个 compute_* 从 usage.jsonl 算) + ascii_render.py(Unicode 块 sparkline ▁▂▃▄▅▆▇█)。6 条曲线全部支持空数据降级(`insufficient` 标志 + 可执行提示)。text + json 输出。C5 / C2 仍是代理形式,等 T-189 / 跨任务聚类完整实现。11 单测,整库 722 |
