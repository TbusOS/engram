...
def test_prompt_budget_drops_sessions_past_cap() -> None:
    first = _sfd("first", body="F" * 400)
    second = _sfd("second", body="S" * 400)
    p = build_distill_prompt([first, second], header="H", max_bytes=600)
    assert "first" in p
    assert "second" not in p


def test_prompt_budget_is_hard_cap_for_single_oversized_session() -> None:
    s = _sfd("huge", body="X" * 10_000)
    p = build_distill_prompt([s], header="H", max_bytes=2_000)
    assert len(p.encode("utf-8")) <= 2_000
    assert "huge" in p  # session header survives; body is truncated
