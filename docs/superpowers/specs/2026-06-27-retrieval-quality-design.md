...
- ~~RRF × Stage-5 multiplier composition (found in step-3 review)~~ **RESOLVED 2026-06-29 (§11): harmonic-rank base.** Extended the goldset with 3 scope + 2 temporal mixed-scope/date queries, measured five base-scaling options through the real gate, and chose `base = 1/rank` (scope MRR 0.33→1.00, temporal →1.00, relevance unchanged). The fused path is now correct; caller wiring is unblocked.
