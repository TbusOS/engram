...
- ~~Fused exact-relevance tie at the rank-1/2 boundary (step-3 review MEDIUM)~~ **RESOLVED 2026-06-29 (before caller wiring).** Root cause was deeper than the rank-1/2 gap: `_ranks` broke score ties by id, so even *exactly* equally-relevant docs got distinct ranks → distinct RRF → id decided the top. Fix is two-part: (1) `_ranks` uses **competition ranking** (equal scores share a rank, no id tiebreak) so equal relevance yields equal RRF; (2) the fused order settles RRF ties by **scope then enforcement then id**. Net: equal relevance → scope decides (project beats org, input-order-independent); distinct relevance never reaches the tiebreak, so a genuinely more-relevant doc is never flipped. The chosen fix needs no offset retuning (harmonic stays `1/rank`). Calibration scope/temporal/relevance unchanged (1.00 / 1.00 / 0.733). Locked by `test_fused_equal_relevance_scope_decides_not_id`.
