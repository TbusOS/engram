...
- **Semantic signal = local-default (fastembed / ONNX, `bge-m3` + `bge-reranker-v2-m3`), optional remote, BM25 fallback** (user, 2026-06-27). Rationale: reranking is the biggest quality lever and the canonical strong reranker is a local cross-encoder; a pinned local model keeps the ratchet reproducible; hosted APIs drift and are not local. Remote is an opt-in ceiling-raiser.
